...
fn test_face_detection_inference() -> Result<()> {
    ...
    let test_image =
        Mat::new_rows_cols_with_default(480, 640, CV_8UC3, opencv::core::Scalar::new(128.0, 128.0, 128.0, 0.0))?;
...
fn test_landmark_detection_inference() -> Result<()> {
    ...
    let test_face =
        Mat::new_rows_cols_with_default(112, 112, CV_8UC3, opencv::core::Scalar::new(128.0, 128.0, 128.0, 0.0))?;
...
fn test_full_pipeline() -> Result<()> {
    ...
    let test_image = if Path::new("test_face.jpg").exists() {
        ...
        let mut img =
            Mat::new_rows_cols_with_default(480, 640, CV_8UC3, opencv::core::Scalar::new(200.0, 200.0, 200.0, 0.0))?;
...
fn test_model_metadata() -> Result<()> {
    ...
    println!(
        "Landmark detector model size: {} MB",
        landmark_detector_size / 1_000_000
    );
    ...
    assert!(
        landmark_detector_size < 100_000_000,
        "Landmark detector model too large"
    );
