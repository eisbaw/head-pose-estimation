...
fn test_filter_output_accuracy() {
    ...
    // Test no filter (passthrough)
    {
        ...
        let test_values = vec![(1.23, 4.56), (-10.0, 20.0), (0.0, 0.0), (100.0, -100.0)];
...
fn test_filter_convergence_accuracy() {
    ...
    // Test that moving average converges to constant input
    {
        ...
        assert_eq!(
            (x, y),
            (33.0, 66.0),
            "Moving average of constant values should equal the constant"
        );
    }
}
    ...
    // Test low-pass filter response to step input
    {
        ...
        for i in 1..responses.len() {
            assert!(
                ...
                "Low-pass filter should monotonically approach step input"
            );
        }

        // Check specific values (exponential response)
        assert!(
            (responses[0] - 10.0).abs() < 0.001,
            "First response should be alpha * input"
        );
        assert!(
            responses[9] > 65.0,
            "Should be significantly closer to target after 10 steps"
        );
    }
}
...
fn test_hampel_filter_accuracy() {
    ...
    assert!(
        x_outlier < 100.0,
        "Hampel filter should reject outlier, got {}",
        x_outlier
    );
    assert!(
        y_outlier < 100.0,
    ...
    assert!(
        (x_normal - 70.0).abs() < 20.0,
        "Should accept normal value close to trend"
    );
    assert!(
        (y_normal - 70.0).abs() < 20.0,
        "Should accept normal value close to trend"
    );
}
...
fn test_second_order_lowpass_accuracy() {
    ...
    assert!(
        x_vals[49] > 85.0,
        "Should converge close to target value, got {}",
        x_vals[49]
    );
    ...
    for i in 5..49 {
        assert!(
            x_vals[i + 1] >= x_vals[i] - 0.001,
            "Should increase monotonically with damping = 0.7"
        );
    }
}
...
fn test_kalman_filter_accuracy() {
    ...
    assert!(
        (mean - true_value).abs() < 2.0,
        "Kalman filter mean {} should be close to true value {}",
        mean,
        true_value
    );

    // Calculate variance
    let variance = filtered_values.iter().map(|&v| (v - mean).powi(2)).sum::<f64>() / filtered_values.len() as f64;
