...
use head_pose_estimation::{
    face_detection::FaceDetector, filters::create_filter, mark_detection::MarkDetector,
    movement_detector::MovementDetector, pose_estimation::PoseEstimator,
};
...
fn test_full_pipeline() {
    ...
    let mut face_detector =
        FaceDetector::new("assets/face_detector.onnx", 0.7, 0.3).expect("Failed to create face detector");
    ...
    let test_image = Mat::zeros(480, 640, opencv::core::CV_8UC3).unwrap().to_mat().unwrap();
    ...
    for _face in faces {
        ...
        let landmarks = mark_detector.detect(&test_image).expect("Landmark detection failed");
...
fn test_filter_pipeline() {
    ...
    for filter_name in filter_names {
        ...
        for &(pitch, yaw) in &test_data {
            ...
            assert!(
                filtered.0.is_finite(),
                "Filter {} produced non-finite pitch",
                filter_name
            );
...
fn test_movement_detection_integration() {
    ...
    for i in 0..60 {
        ...
        if i >= 30 {
            assert!(
                !is_moving,
                "Should not detect movement for still pose after window is filled"
            );
        }
    }
