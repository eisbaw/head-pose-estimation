    ...
    for i in 0..test_duration {
        ...
        for (filter_idx, filter) in filters.iter_mut().enumerate() {
            ...
            if !filtered_pitch.is_finite() || !filtered_yaw.is_finite() {
                return Err(head_pose_estimation::Error::FilterError(
                    format!("Filter {} produced non-finite values", filter_idx)
                ));
            }
            
            if filtered_pitch.abs() >= 100.0 || filtered_yaw.abs() >= 100.0 {
                return Err(head_pose_estimation::Error::FilterError(
                    format!("Filter {} produced values out of reasonable range", filter_idx)
                ));
            }
        }
    }
...
fn test_filter_thread_safety_safe() -> Result<()> {
    ...
    for thread_id in 0..4 {
        ...
        let handle = thread::spawn(move || -> Result<()> {
            for i in 0..25 {
                ...
                let mut filter = filter_clone.lock()
                    .map_err(|e| head_pose_estimation::Error::FilterError(
                        format!("Failed to lock filter: {}", e)
                    ))?;
    ...
    for handle in handles {
        handle.join()
            .map_err(|_| head_pose_estimation::Error::FilterError(
