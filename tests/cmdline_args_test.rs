...
fn create_test_command() -> ClapCommand {
    ClapCommand::new("head-pose-estimation")
        ...
        .arg(Arg::new("movement-based-cursor")
            .long("movement-based-cursor")
            .action(ArgAction::SetTrue)
            .help("Enable movement-based cursor"))
        .arg(Arg::new("show-all-filters")
            .long("show-all-filters")
            .action(ArgAction::SetTrue)
            .help("Show all filters"))
        .arg(Arg::new("invert-x")
            .long("invert-x")
            .action(ArgAction::SetTrue)
            .help("Invert X axis"))
        .arg(Arg::new("invert-y")
            .long("invert-y")
            .action(ArgAction::SetTrue)
            .help("Invert Y axis"))
        .arg(Arg::new("use-movement-filter")
        ...
        .arg(Arg::new("cursor-sensitivity")
...
fn test_no_arguments() {
    ...
    assert_eq!(matches.get_one::<String>("data-source").map(|s| s.as_str()), Some("angles"));
}
...
fn test_data_source_arguments() {
    ...
    for source in sources {
        ...
        let matches = result.unwrap();
        assert_eq!(matches.get_one::<String>("data-source").map(|s| s.as_str()), Some(source));
    }
}
...
fn test_numeric_arguments() {
    ...
    assert!(result.is_ok());
    let matches = result.unwrap();
    assert_eq!(matches.get_one::<String>("cursor-sensitivity").map(|s| s.as_str()), Some("2.0"));
}
...
fn test_config_file_argument() {
    ...
    assert!(result.is_ok());
    let matches = result.unwrap();
    assert_eq!(matches.get_one::<String>("config").map(|s| s.as_str()), Some("config.yaml"));
}
...
fn test_multiple_arguments() {
    ...
    assert_eq!(matches.get_one::<String>("data-source").map(|s| s.as_str()), Some("speed"));
