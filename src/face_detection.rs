...
impl FaceDetector {
    ...
    pub fn new<P: AsRef<Path>>(model_path: P, conf_threshold: f32, nms_threshold: f32) -> Result<Self> {
        ...
        let (offset, strides, num_anchors, with_kps) = match num_outputs {
            ...
            _ => {
                log::warn!(
                    "Unknown model configuration with {} outputs, using defaults",
                    num_outputs
                );
    ...
    pub fn detect(&mut self, image: &Mat) -> Result<Vec<FaceDetection>> {
        ...
        // Create padded image
        let mut det_img =
            Mat::new_rows_cols_with_default(input_height, input_width, opencv::core::CV_8UC3, Scalar::all(0.0))?;
    ...
    fn forward(&mut self, inputs: Array4<f32>, threshold: f32) -> Result<FaceDetectionOutputs> {
        ...
        for (idx, &stride) in self.strides.iter().enumerate() {
            ...
            let scores_flat = scores_view.as_slice().ok_or_else(|| {
                crate::error::Error::ModelOutputError(format!(
                    "Failed to extract scores as slice for stride {stride} at output index {idx}"
                ))
            })?;
            ...
            let bbox_slice = bbox_view.as_slice().ok_or_else(|| {
                crate::error::Error::ModelOutputError(format!(
                    "Failed to extract bbox data as slice for stride {stride} at output index {bbox_idx}"
                ))
            })?;
            let bbox_data: Vec<f32> = bbox_slice.iter().map(|&x| x * stride as f32).collect();
            ...
            let pos_bboxes = Array2::from_shape_vec(
                (pos_inds.len(), 4),
                pos_inds.iter().flat_map(|&i| decoded_bboxes.row(i).to_vec()).collect(),
            )
            ...
            if self.with_kps {
                ...
                let kps_slice = kps_view.as_slice().ok_or_else(|| {
                    crate::error::Error::ModelOutputError(format!(
                        "Failed to extract keypoints data as slice for stride {stride} at output index {kps_idx}"
                    ))
                })?;
                let kps_data: Vec<f32> = kps_slice.iter().map(|&x| x * stride as f32).collect();
    ...
    fn generate_anchor_centers(&self, height: i32, width: i32, stride: i32) -> Result<Array2<f32>> {
        ...
        Array2::from_shape_vec((n_points, 2), centers).map_err(|e| {
            crate::error::Error::ModelDataFormatError(format!("Failed to create anchor centers array: {e}"))
        })
    }
    ...
    fn postprocess(
        ...
        for &idx in &keep {
            ...
            let keypoints = if self.with_kps && !kpss_list.is_empty() {
                ...
                Some(
                    (0..5)
                        .map(|i| Point2f::new(kps[[i, 0]] / det_scale, kps[[i, 1]] / det_scale))
                        .collect(),
                )
            } else {
                None
            ...
            detections.push(FaceDetection { bbox, score, keypoints });
        }
    ...
    fn nms(&self, bboxes: &Array2<f32>, _scores: &Array1<f32>, order: &[usize]) -> Vec<usize> {
    ...
    fn distance_to_kps_array(points: &Array2<f32>, distances: &[f32], max_shape: Option<(i32, i32)>) -> Array3<f32> {
