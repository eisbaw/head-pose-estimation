...
impl Config {
    ...
    pub fn create_filter(&self) -> Result<Box<dyn crate::filters::CursorFilter>> {
        use crate::filters::{
            create_filter, exponential::ExponentialFilter, hampel::HampelFilter,
        ...
        match self.filter.default_filter.as_str() {
            ...
            "median" => Ok(Box::new(MedianFilter::new(
                self.filter.median_window
            ))),
            "exponential" => Ok(Box::new(ExponentialFilter::new(
                self.filter.exponential_alpha
            ))),
            "low_pass" => Ok(Box::new(LowPassFilter::new(
                self.filter.low_pass_alpha
            ))),
