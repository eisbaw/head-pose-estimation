...
fn benchmark_filters(c: &mut Criterion) {
    ...
    let filter_configs = vec![
        ...
        ("second_order_low_pass", Box::new(SecondOrderLowPassFilter::new(10.0, 0.7))),
    ...
    for (name, mut filter) in filter_configs {
        ...
        group.bench_with_input(
            BenchmarkId::new("sequence_100", name),
            &test_data,
            |b, data| {
                b.iter(|| {
                    filter.reset();
...
fn benchmark_filter_reset(c: &mut Criterion) {
    ...
    let filter_configs = vec![
        ("moving_average", Box::new(MovingAverageFilter::new(10)) as Box<dyn CursorFilter>),
