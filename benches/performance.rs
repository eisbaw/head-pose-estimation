...
fn bench_filters(c: &mut Criterion) {
    ...
    for filter_name in filter_names {
        group.bench_with_input(BenchmarkId::new("apply", filter_name), &filter_name, |b, &name| {
...
fn bench_filter_reset(c: &mut Criterion) {
    ...
    let filter_names = vec!["moving_average", "median", "exponential", "kalman", "lowpass", "hampel"];

    for filter_name in filter_names {
        group.bench_with_input(BenchmarkId::new("reset", filter_name), &filter_name, |b, &name| {
            let mut filter = create_filter(name).unwrap();
...
fn bench_movement_detection(c: &mut Criterion) {
    ...
    for window_size in window_sizes {
        group.bench_with_input(BenchmarkId::new("update", window_size), &window_size, |b, &size| {
...
fn bench_filter_accuracy_vs_speed(c: &mut Criterion) {
    ...
    let noisy_signal: Vec<(f64, f64)> = clean_signal
        .iter()
        .enumerate()
        .map(|(i, &(pitch, yaw))| {
            ...
            (pitch + gaussian_noise * 3.0 + spike, yaw + gaussian_noise * 3.0 - spike)
        })
        .collect();
    ...
    for (name, mut filter) in filters {
        group.bench_with_input(BenchmarkId::new("process_10k_samples", name), &name, |b, _| {
            b.iter(|| {
                filter.reset();
